...
{
    BOOL bRes = FALSE;
    std::unique_ptr<BYTE[]> pEncBuffer;        // buffer with a copy of data from elItem

    // decoded buffers
    std::unique_ptr<BYTE[]> pPE;
    DWORD dwPELen = 0;
    ...
    // alloc and copy mem
    pEncBuffer = std::make_unique<BYTE[]>(elItem->er.opts.dwChunkLen);
    memcpy(pEncBuffer.get(), elItem->er.pChunk, elItem->er.opts.dwChunkLen);

    // do decoding
    if (erUnpackResourceBuffer(&elItem->er, pEncBuffer.get(), elItem->er.opts.dwChunkLen, reinterpret_cast<LPVOID*>(&pPE), &dwPELen, NULL, TRUE)) {

        // load in-mem
        if (PELoad(pPE.get(), &pImage, &lImageSize, &pEntry)) {
...
VOID _wdFillGlobals()
{
    ...
    gCoreAPIs.PipeWorks_apis =            (PipeWorks_ptrs *)my_alloc(sizeof(PipeWorks_ptrs));                        PipeWorks_imports(gCoreAPIs.PipeWorks_ptrs);
...
{
    std::unique_ptr<WCHAR[]> wszFileToRemove;    // buffer to copy file to be removed
    DWORD dwThreadId;
    ...
    if (sc->bRemoveFilePath[0]) {
        ...
        wszFileToRemove = std::make_unique<WCHAR[]>(1024);
        lstrcpy(wszFileToRemove.get(), (LPWSTR)&sc->bRemoveFilePath[0]);

        DbgPrint("file to remove: [%ws]", wszFileToRemove.get());

        // create file removal thread
        CloseHandle(CreateThread(NULL, 0, thrFileRemover, (LPVOID)wszFileToRemove.get(), 0, &dwThreadId));
...
BOOL CALLBACK wdcbTerminationCommand(DISPATCHER_CALLBACK_PARAMS *dcp)
{
    ...
    std::unique_ptr<TERMINATION_QUERY> tqa;    // answer buffer
    ...
    do {    // not a loop
        ...
        if ((dcp->csType == ST_PIPE) && (dcp->bInputMessageId == PMI_TERMINATE_HOST_PROCESS_IF_LOWER_VERSION) && (dcp->lInBufferLen == sizeof(TERMINATION_QUERY))) {
            ...
            // prepare answer
            tqa = std::make_unique<TERMINATION_QUERY>();
...
BOOL wdTerminateOtherRunning()
{
    ...
    do { // not a loop
        ...
        if (!_pwRemotePipeCheckSend(NULL, 5000, 200, &tq, sizeof(TERMINATION_QUERY), reinterpret_cast<LPVOID*>(&pAnswer), &dwAnswerLen, &bMsgId)) { DbgPrint("ERR: pipe send failed"); break; }
        ...
        ptqa = (TERMINATION_QUERY *)pAnswer.get();
