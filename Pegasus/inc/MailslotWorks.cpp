...
VOID mwInitMailslotServer(CLIENTDISPATCHERFUNC cdCallback)
{
    ...
    if (mip->hSlot == INVALID_HANDLE_VALUE) { 
        DbgPrint("ERR: CreateMailslot() failed %04Xh", GetLastError()); 
    ...
    if (!SetHandleInformation(mip->hSlot, HANDLE_FLAG_INHERIT, 0)) {
        DbgPrint("ERR: SetHandleInformation() failed %04Xh", GetLastError());
        CloseHandle(mip->hSlot);
        my_free(wszMailslotName);
        my_free(mip);
        return;
    ...
    if (hThread) { 
        ...
        DbgPrint("ERR: failed to create dispatcher thread, le %04Xh", GetLastError()); 
        CloseHandle(mip->hSlot);
        my_free(mip);
    }
...
BOOL mwSendMailslotMessageToDomain(LPVOID pBuffer, DWORD dwMessageLen, LPWSTR wszTargetDomain, BYTE bMailslotMessageId)
{
    ...
    if (hSlot != INVALID_HANDLE_VALUE) {
        ...
        if (!WriteFile(hSlot, pEnveloped, dwEnvelopedLen, &dwWritten, NULL)) { 
            DbgPrint("ERR: WriteFile() to mailslot failed le %04Xh, msg_len=%u", GetLastError(), dwEnvelopedLen); 
        } else { 
            /* DbgPrint("[%ws] ok sent %u bytes of %u", wszTargetDomain, dwWritten, dwEnvelopedLen); */ 
            bRes = TRUE; 
        }
